...
function state(initialValue) {
  ...
  const stateName = statePaths.find((state) => state.stateNumber === stateNumber).stateName;
  ...
  const handler = {
    ...
    set(target, prop, value, receiver) {
      ...
      states.set(stateNumber, target);
      updateStateUI(stateNumber, prop);
      return result;
    }
  };
  ...
  Object.defineProperty(window, stateName, {
    ...
    set(value) {
      ...
      const updatedValue = (typeof value === 'object' && value !== null)
        ...
        : value;
      states.set(stateNumber, updatedValue);
      updateStateUI(stateNumber);
    }
  });
...
function updateStateUI(stateNumber, changedProperty) {
  ...
  const replacePlaceholder = (value) => {
    return value.replace(/{(\w+(\.\w+|\['\w+'\])?)}/g, (match, name) => {
      ...
      let stateObj = states.get(getStateNumberByName(state));
